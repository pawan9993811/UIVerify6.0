...
namespace VisualUIAVerify.Controls
{
    ...
    public partial class AutomationElementTreeControl : UserControl
    {
        ...
        //private static AutomationElementTreeControl instance;

        //public static AutomationElementTreeControl Instance
        //{
        //    get
        //    {
        //        if (instance == null)
        //            instance = new AutomationElementTreeControl();
        //        return instance;
        //    }
        ...
        public List<string> PrintItems(List<UIControl> items, int levelOfIndentation)
        {
            List<string> lst = new List<string>();
            foreach (UIControl anItem in items)
            {
                // print the currentItem
                lst.Add(anItem.Name);
                // increment the level of indentation and callk the same method for the children
                return PrintItems(items, levelOfIndentation + 1);
            }
            return new List<string>();
        }
        //private UIControl GetTopParent(UIControl control)
        //{
        //    if (control.Parent != null)
        //        GetTopParent(control.Parent);
        //    return control;
        //}

        private void CreateXML(List<TreeNode> nodeHierarchyElements)
        {
            ...
            openXML(nodeHierarchyElements);
        }

        private UIControl ConstructUIControlFromXMLNode(UIControl control, TreeNode nodeElement)
        {
            if (control == null)
            {
                control = new UIControl();
                //Add Technology and ApplicationType
                // Siva these values should be set based on list item you have selected
                control.ApplicationType = ApplicationTypes.Windows;
                control.TechnologyType = TechnologyTypes.White;

                var aeTreeNode = (AutomationElementTreeNode)nodeElement.Tag;
                var currentNodeAE = aeTreeNode.AutomationElement;

                //Set Display name to UIControl
                SetUIControlDisplayName(control, currentNodeAE, FileName);

                //Set Search Properties to UIControl
                SetSearchProperties(control, currentNodeAE);
                return control;
            }
            else
            {
                UIControl childControl = new UIControl();

                //Add Technology and ApplicationType, // Siva these values should be set based on list item you have selected
                control.ApplicationType = ApplicationTypes.Windows;
                control.TechnologyType = TechnologyTypes.White;
                ...
                //Set Display name to UIControl
                SetUIControlDisplayName(childControl, currentNodeAE, FileName);
                childControl.ApplicationType = ApplicationTypes.Windows;
                childControl.TechnologyType = TechnologyTypes.White;

                //Set Search Properties to UIControl
                SetSearchProperties(childControl, currentNodeAE);
                control.ChildNodes.Add(childControl);
                //control.ChildNodes.First().Parent = control;
        ...
        private void SetUIControlDisplayName(UIControl control, AutomationElement ae, string xmlFilePath)
        {
            ...
            {
                control.Name = ae.Current.Name;
            }
            else if (!string.IsNullOrEmpty(ae.Current.ClassName))
            {
                control.Name = ae.Current.ClassName;
            }
            else if (!string.IsNullOrEmpty(ae.Current.AutomationId))
            {
                control.Name = ae.Current.AutomationId;
            }
            else if (!string.IsNullOrEmpty(ae.Current.LocalizedControlType))
            {
                control.Name = ae.Current.LocalizedControlType;
            }
            else
            {
                control.Name = "UIControl_" + randomNumber;
            }

            CheckandSetDisplayName(displayNamesList, control.Name);

            displayNamesList.Add(control.Name);

        }

        private string CheckandSetDisplayName(List<string> displayNamesList, string displayName)
        {
            Random random = new Random();
            int randomNumber = random.Next(0, 1000);

            foreach (string name in displayNamesList)
            {
                if (name.Equals(displayName))
                    displayName = displayName + randomNumber.ToString();
            }

            return displayName;
        }

        private void CheckAndAddControlToUIView(UIControl RepoControlNode, UIControl liveUIControl)
        {
            int index = 0;
            if ((RepoControlNode.ChildNodes.Count == 0) && (liveUIControl.ChildNodes.Count == 0))
                RepoControlNode.Equals(liveUIControl);
            else
                CompareControl(RepoControlNode.ChildNodes, liveUIControl, out index);

            CheckAndAddControlToUIView(RepoControlNode.ChildNodes[index], liveUIControl.ChildNodes.First());


            //if(RepoControlNode.Equals(liveUIControl))
            //    CheckAndAddControlToUIView(RepoControlNode.ChildNodes.First)


        }


        private void CompareControl(List<UIControl> UIControlCollection, UIControl control, out int index)
        {
            index = 0;
            foreach (UIControl ctrl in UIControlCollection)
            {
        ...
        private void SetSearchProperties(UIControl control, AutomationElement ae)
        {
            if (!string.IsNullOrEmpty(ae.Current.AutomationId))
            {
                control.SearchCriteria.SearchProperties.Add("AutomationID", ae.Current.AutomationId, "EqualsTo");
            }
            if (!string.IsNullOrEmpty(ae.Current.ClassName))
            {
                control.SearchCriteria.SearchProperties.Add("ClassName", ae.Current.ClassName, "EqualsTo");
            }
            if (!string.IsNullOrEmpty(ae.Current.Name))
            {
                control.SearchCriteria.SearchProperties.Add("Name", ae.Current.Name, "EqualsTo");
            }
            if (!string.IsNullOrEmpty(ae.Current.LocalizedControlType))
            {
                control.SearchCriteria.SearchProperties.Add("ControlType", ae.Current.LocalizedControlType, "EqualsTo");
            }
        }
